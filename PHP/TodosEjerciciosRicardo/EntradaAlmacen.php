...
<body>
    ...
    if (isset($_POST['Stock']))    //Si hay que registrar el producto en la tabla Stock
    {
        ...
        $stock2 = $daoStock->obtener($sto);   //Recuperamos el objeto para ese stock
    
        if ($stock2 == null) {
            $daoStock->insertar($sto);
        } else //Si hay que actualziar el stock
        {
            $cantidad += $stock2->__get('unidades');  //A la cantidad que entra hay que sumarle la que ya teniamos
    
            $sto->__set("unidades", $cantidad);

            $daoStock->actualizar($sto);
        }
