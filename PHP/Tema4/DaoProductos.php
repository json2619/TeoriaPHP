...
class DaoProductos extends DB
{
    ...
    public function listFamNom($familia = "", $nombre = "", $campo = "", $orden = "")       //Lista los productos de una familia y que coincida el nombre
    {
        ...
        if ($campo != '')   // si hemos recibido un código de familia
        {
            $consulta .= " ORDER by $campo ";
        }

        if ($orden != '')   // si hemos recibido un código de familia
        {
            if ($orden == 1) {
                $consulta .= " ASC ";
            } else {
